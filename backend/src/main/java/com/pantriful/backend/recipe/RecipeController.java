...
public class RecipeController {
    ...
    // PUT/DELETE (rather than a toggle) so repeated taps or retries always land on the requested state.
    @PutMapping("/{id}/favorite")
    public RecipeResponse favorite(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID id) {
        return recipeService.setFavorite(userId(jwt), id, true);
    }

    @DeleteMapping("/{id}/favorite")
    public RecipeResponse unfavorite(@AuthenticationPrincipal Jwt jwt, @PathVariable UUID id) {
        return recipeService.setFavorite(userId(jwt), id, false);
    }
